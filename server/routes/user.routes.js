...
const Order = require("../models/Order");  // ✅ This is the only import needed
...
router.get("/me", auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      ...
      .select("name email role cart wishlist orders avatar addresses");
...
router.get("/all", auth, async (req, res) => {
  try {
    ...
    // 2. Check role case-insensitively
...
// ✅ DELETE USER (Admin Only - Fixed 403 Logic)
...
router.get("/search", auth, async (req, res) => {
  try {
    ...
    const role = requestingUser.role.toLowerCase();
    if (role !== 'admin' && role !== 'superadmin') return res.status(403).json({ message: "Access denied" });
...
// ✅ GET USER ADDRESSES (Admin View - NEW)
router.get("/admin/:userId/addresses", auth, async (req, res) => {
  try {
    const requestingUser = await User.findById(req.user.id);
    if (!requestingUser) return res.status(403).json({ message: "Access denied" });
...
// ✅ GET USER ORDERS (Admin View - NEW)
router.get("/admin/:userId/orders", auth, async (req, res) => {
  try {
    const requestingUser = await User.findById(req.user.id);
    if (!requestingUser) return res.status(403).json({ message: "Access denied" });
