...
// Define the Order type to match what the backend sends for '/my-orders'
interface Order {
  _id: string;
  totalAmount: number;
  createdAt: string;
  ...
  // Include the full object for passing to the tracking page
  shipmentDetails?: {
      awbCode?: string;
      courierName?: string;
  };
...
const Orders = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserOrders = async () => {
      try {
        setLoading(true);
        // This route fetches all orders for the logged-in user
            ...
            <div key={order._id} className="bg-white p-4 rounded-lg shadow-md transition hover:shadow-lg">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                   <img src={order.cartItems[0]?.image} alt={order.cartItems[0]?.name} className="w-16 h-16 object-cover rounded-md hidden sm:block" />
                   <div>
                    <h3 className="font-semibold text-lg">{order.cartItems[0]?.name}</h3>
                    {order.cartItems.length > 1 && (
                      <p className="text-xs text-gray-500">+ {order.cartItems.length - 1} more item(s)</p>
                    )}
                    <p className="text-sm font-bold text-green-600 mt-2">Status: {order.status}</p>
                   </div>
                </div>
                {/* This button passes the entire order object to the tracking page */}
                <Link
                  to={`/track-order`}
                  state={{ order: order }}
                  className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium whitespace-nowrap hover:bg-gray-100"
                >
                  Track Order
                </Link>
              </div>
